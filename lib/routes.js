...
module.exports = function (app) {
	...
	app.get('/storedPosts', function (req, res, next) {
		database.query("select * from adminPosts", function (err, result, fields) {
			console.log('Goku: ', JSON.stringify(result));
		});
	});
	...
	app.post('/admin', function (req, res, next) {
		...
		database.query("INSERT INTO adminPosts VALUES("+mysql.escape(postId)+","+mysql.escape(req.body.title)+","+mysql.escape(req.body.content)+","+mysql.escape(req.body.cta)+","+mysql.escape(req.body.ctaName)+","+mysql.escape(authenticationStatus)+")");
		res.redirect('/admin');
	});
